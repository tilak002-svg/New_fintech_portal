(function () {
    'use strict';
    const { apiFetch, showToast, renderTrendChart, escapeHtml, formatMoney: money, setButtonLoading, openModal, closeModal, formatISTDate } = window.Verapay;
    ...
    function renderRecentActivity(data) {
        ...
        tbody.innerHTML = data.recent.map((t) => `
            <tr class="border-l-4 ${statusBorderClass(t.status)}">
                ...
                <td class="text-text-secondary">${formatISTDate(t.created_at)}</td>
            </tr>`).join('');
    }
